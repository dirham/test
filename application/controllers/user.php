...
class User extends App_Crud_Controller {
    ...
    function _config_grid() {
        ...
        $config['fields'] = array('username','name','year_data', 'created_by');
        $config['names'] = array('NIM', 'Nama Mahasiswa','Tahun Akademik', 'Diisi Oleh');
        $config['styles'] = array('', '',  '', '', '');
    ...
    function index() {
        $this->load->helper('format');
        $user = $this->db->query('
            SELECT * ,user.id, user.name, user.image FROM user
    ...
    function _save_nilai_semester_1($mahasiswa_id, $id = null) {
        ...
        $matkuls = $this->db->query('SELECT * FROM matkul ORDER BY semester')->result_array();
